...
Tag,Finding (Vulnerability Type),Impact
B-01,"Unsafe External Call: Uses a bare transfer for an ERC20 withdrawal, lacking robust error handling (no check for false return value).",Medium
B-02,"Event Data Integrity: The logging event for token withdrawal has two key indexed parameters swapped, leading to confusing or incorrect off-chain data indexing.",Low
B-03,"Critical Initialization Error: The pool validation logic fails to check for one of the two possible currency positions, effectively blocking legitimate pool initialization.",High
B-04,Event Calculation Error: An arithmetic error (wrong denominator) in the fee calculation for an emitted event leads to inaccurate data reporting.,Low
B-05,"Economic/Logic Inversion: The core business logic for determining the swap direction is inverted in one scenario, causing buyers to be charged the high ""sell"" fee and sellers to receive the low ""buy"" fee.",High
